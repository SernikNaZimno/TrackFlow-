import amqp from 'amqplib'
import { prisma } from './lib/prisma'
import { handleClickRecorded } from './consumers/click.consumer'
import { handleNotificationSend } from './consumers/notification.consumer'
import { initWeeklyReportCron } from './cron/weekly-report.cron'
import { initInactivityAlertCron } from './cron/inactivity-alert.cron' // Nowy import

async function start() {
  console.log('Uruchamianie Workera TrackFlow...')
  
  try {
    const rabbitUrl = process.env.RABBITMQ_URL || 'amqp://trackflow:trackflow@localhost:5672'
    const connection = await amqp.connect(rabbitUrl)
    const channel = await connection.createChannel()
    
    await channel.assertExchange('trackflow.events', 'topic', { durable: true })
    initWeeklyReportCron(channel)
    initInactivityAlertCron(channel)
    // 1. Kolejka kliknięć (Z POPRZEDNIEGO KROKU)
    await channel.assertQueue('trackflow.clicks', { durable: true })
    await channel.bindQueue('trackflow.clicks', 'trackflow.events', 'click.recorded')

    // 2. NOWOŚĆ: Kolejka powiadomień
    await channel.assertQueue('trackflow.notifications', { durable: true })
    await channel.bindQueue('trackflow.notifications', 'trackflow.events', 'notification.send')

    await channel.prefetch(1)
    await prisma.$connect()

    // Odpalenie konsumerów
    channel.consume('trackflow.clicks', (msg) => handleClickRecorded(msg, channel), { noAck: false })
    channel.consume('trackflow.notifications', (msg) => handleNotificationSend(msg, channel), { noAck: false })
    
    console.log('Worker nasłuchuje zdarzeń RabbitMQ. Gotowy do pracy.')
  } catch (error) {
    console.error('Krytyczny błąd podczas startu Workera:', error)
    process.exit(1)
  }
}

start()