...
const postLinkSchema = z.object({
  original_url: z.string().url('Nieprawidłowy URL').max(2048),
  campaign_name: z.string().max(255).nullable().optional(),
...
export default async function linksRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/links', async (request, reply) => {
    ...
    if (client_id) {
      const clientUser = await prisma.user.findUnique({ where: { id: client_id } })
    ...
    while (attempts < 3 && !savedLink) {
      ...
      try {
        savedLink = await prisma.link.create({
          data: {
            shortCode,
  ...
  fastify.patch('/api/links/:id', async (request, reply) => {
    ...
    // Inwalidacja w Redis!
    await redis.del(`link:${link.shortCode}`).catch(() => {})
